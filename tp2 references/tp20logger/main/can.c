...
void CAN_Open()
{
    ...
    twai_general_config_t g_config = TWAI_GENERAL_CONFIG_DEFAULT(GPIO_NUM_26, GPIO_NUM_32, TWAI_MODE_NORMAL);
...
uint8_t CAN_ReceiveMsg(CanMessage_t * msg, int timeout){
    ...
    esp_err_t err = twai_receive(&_msg, portTICK_PERIOD_MS * timeout);
    ...
    printf("%s:%d FAILED: %x fetching message\n", __FILE__, __LINE__,err);
...
uint8_t CAN_SendMsg(CanMessage_t * msg){
    ...
    esp_err_t err = twai_transmit(&_msg, portMAX_DELAY);
    if(err == ESP_OK)
    {
...
void CAN_SetFilter0(uint32_t id){
    ...
    twai_filter_config_t f_config = {.acceptance_code = (id << 21),
