...
class TestOpenpilotTP2(unittest.TestCase):
    ...
    def test_full_flow(self):
        ...
        # 2. Wait for Parameters Request (A0) on 0x67A from the receiver
        msg_params_req = self.comma_bus.recv(timeout=1.0)
        self.assertIsNotNone(msg_params_req)
        self.assertEqual(msg_params_req.arbitration_id, 0x67A)
        self.assertEqual(msg_params_req.data[0], 0xA0)
        
        # Send Parameters Response (A1) on 0x6DA
        params_resp = [0xA1, 0x0F, 0x8A, 0xFF, 0x4A, 0xFF]
        msg_params_resp = can.Message(arbitration_id=0x6DA, data=params_resp, is_extended_id=False)
        ...
        # 3. Stream Message Type 1 (Fast State & 3 Leads @ 10Hz)
        ...
        self.comma_bus.send(can.Message(arbitration_id=0x6DA, data=f1_data, is_extended_id=False))
        
        # Frame 2: Opcode 0x21 (more packets)
        f2_data = [0x21] + fast_payload[5:12]
        self.comma_bus.send(can.Message(arbitration_id=0x6DA, data=f2_data, is_extended_id=False))
 
        # Frame 3: Opcode 0x32 (last frame)
        f3_data = [0x32] + fast_payload[12:13] + [0, 0, 0, 0, 0, 0, 0]
        self.comma_bus.send(can.Message(arbitration_id=0x6DA, data=f3_data, is_extended_id=False))
        ...
        # 4. Stream Message Type 2 (Path/Lanes State @ 10Hz)
        ...
        self.comma_bus.send(can.Message(arbitration_id=0x6DA, data=f1_path, is_extended_id=False))
        
        # Frame 2: Opcode 0x23 (more packets)
        f2_path = [0x23] + path_payload[5:12]
        self.comma_bus.send(can.Message(arbitration_id=0x6DA, data=f2_path, is_extended_id=False))
        
        # Frame 3: Opcode 0x34 (last frame)
        f3_path = [0x34] + path_payload[12:23] + [0, 0, 0, 0]
        self.comma_bus.send(can.Message(arbitration_id=0x6DA, data=f3_path, is_extended_id=False))
        ...
        # 5. Test Keep-Alive Ping Response
        # Send A3 on 0x6DA
        self.comma_bus.send(can.Message(arbitration_id=0x6DA, data=[0xA3], is_extended_id=False))
        
        # Expect A1 response on 0x67A
        msg_ka_ack = self.comma_bus.recv(timeout=1.0)
        self.assertIsNotNone(msg_ka_ack)
        self.assertEqual(msg_ka_ack.arbitration_id, 0x67A)
